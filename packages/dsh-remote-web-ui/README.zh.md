...
- **安全**：一个有效的一次性令牌（刷新会使旧链接失效；已接受的令牌不可复用；令牌会过期）。停止会撤销每一台已配对设备与当前令牌——已配对设备在下一次请求时被切断。已配对设备会话默认跨重启持久化：设备表以仅属主可读写入 `~/.dsh/remote-web-ui/paired-devices.json` 并在启动时读回，重启不再强制重新配对（`persistDevices: false` 恢复旧的重启即撤销行为；`devicesFile` 可改存放路径）。当插件 `requirePairingForLan` 门开启（默认）时，每个非 loopback 的 `/api` 请求必须携带有效的已配对设备 cookie，因此二维码是进入暴露在局域网上的 dsh web 的唯一途径。
